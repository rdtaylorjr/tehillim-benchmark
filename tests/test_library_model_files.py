...
from conftest import SCOPE_DIR, semantic_file
...
def test_returns_every_embeddings_file_when_nothing_is_cached(tmp_path: Path) -> None:
    a = _touch(semantic_file(tmp_path, "a", "part-0.parquet"))
    b = _touch(semantic_file(tmp_path, "b", "part-0.parquet"))

    assert uncached_model_paths(tmp_path, set()) == [a, b]


def test_skips_a_model_already_present_in_the_cache(tmp_path: Path) -> None:
    _touch(semantic_file(tmp_path, "a", "part-0.parquet"))
    b = _touch(semantic_file(tmp_path, "b", "part-0.parquet"))

    assert uncached_model_paths(tmp_path, {"a_consonantal"}) == [b]
...
def test_orders_paths_canonically_so_reruns_submit_work_the_same_way(tmp_path: Path) -> None:
    ...
    _touch(semantic_file(tmp_path, "c", "part-0.parquet"))
    _touch(semantic_file(tmp_path, "a", "part-0.parquet"))
    _touch(semantic_file(tmp_path, "b", "part-0.parquet"))
...
def test_finds_files_nested_at_any_partition_depth(tmp_path: Path) -> None:
    deep = _touch(
        tmp_path
        / SCOPE_DIR
        / "domain=syntactic/level=clause/feature=typ/construction=1gram"
        / "part-0.parquet"
    )
...
def test_ignores_a_directory_that_merely_ends_in_parquet(tmp_path: Path) -> None:
    (tmp_path / SCOPE_DIR / "domain=semantic" / "stray.parquet").mkdir(parents=True)
    real = _touch(semantic_file(tmp_path, "a", "part-0.parquet"))
...
def test_excludes_order_shuffle_draws_which_are_the_null_not_models(tmp_path: Path) -> None:
    ...
    lexical = tmp_path / SCOPE_DIR / "domain=lexical/type=lexeme"
    ...
    _touch(lexical / "construction=icf_position4_shuffle1000" / "part-0.parquet")

    assert uncached_model_paths(tmp_path, set()) == [real]


def test_excludes_the_earlier_two_digit_shuffle_generation_as_well(tmp_path: Path) -> None:
    morphological = tmp_path / SCOPE_DIR / "domain=morphological/feature=sp"
    real = _touch(morphological / "construction=1_2gram" / "part-0.parquet")
    _touch(morphological / "construction=1_2gram_shuffle15" / "part-0.parquet")
...
def test_uncached_model_paths_passes_over_a_parquet_outside_any_hive_partition(
    ...
    (tmp_path / SCOPE_DIR / "domain=semantic/model=m").mkdir(parents=True)
    (tmp_path / SCOPE_DIR / "domain=semantic/model=m/vectors.parquet").touch()
    (tmp_path / "stray.parquet").touch()
    real = _touch(semantic_file(tmp_path, "m", "vectors.parquet"))
