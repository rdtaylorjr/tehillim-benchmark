...
def test_random_labels_give_chance_level_auc() -> None:
    ...
    psalm_vectors = {str(i): rng.normal(size=8) for i in range(1, 41)}
    ...
    for i in range(1, 41):
        for j in range(i + 1, 41):
            ...
            pairs.append(
                GenrePair(str(i), str(j), i, j, genre_a, genre_b, same_genre=genre_a == genre_b)
            )
...
def test_evaluate_genre_discrimination_sparse_agrees_with_dense_at_realistic_density() -> None:
    ...
    dim = 512
    psalm_ids = [str(p) for p in range(1, 13)]
...
def test_reports_nan_when_one_side_of_the_comparison_is_empty() -> None:
    ...
    pairs = [GenrePair("1", "2", 1, 2, "Hymn", "Hymn", same_genre=True)]
...
def test_dense_and_matrix_entry_points_return_the_same_report_exactly() -> None:
    ...
    vectors = {str(p): rng.standard_normal(64).astype(np.float32) for p in psalm_ids}
    genres = {p: "A" if p % 3 else "B" for p in psalm_ids}
    pairs = build_genre_pairs(whole_psalm_passages(genres))
...
def test_dense_entry_point_does_not_stack_one_row_per_pair() -> None:
    ...
    dim = 2048
    vectors = {str(p): rng.standard_normal(dim).astype(np.float32) for p in psalm_ids}
    genres = {p: "A" if p % 3 else "B" for p in psalm_ids}
    pairs = build_genre_pairs(whole_psalm_passages(genres))
...
def test_pair_similarities_reads_every_pair_off_one_psalm_matrix() -> None:
    ...
    vectors = {str(p): rng.standard_normal(32).astype(np.float32) for p in range(1, 21)}
    genres = {p: "A" if p % 2 else "B" for p in range(1, 21)}
    pairs = build_genre_pairs(whole_psalm_passages(genres))
...
def test_pair_similarities_drops_pairs_whose_psalms_have_no_vector() -> None:
    ...
    vectors = {str(p): rng.standard_normal(16).astype(np.float32) for p in (1, 2, 3)}
    pairs = build_genre_pairs(whole_psalm_passages({1: "A", 2: "A", 3: "B", 4: "B"}))

    usable, similarities = pair_similarities(pairs, vectors)

    assert all(p.item_a in vectors and p.item_b in vectors for p in usable)
...
def test_pair_similarities_returns_empty_when_no_pair_is_usable() -> None:
    ...
    usable, similarities = pair_similarities(
        build_genre_pairs(whole_psalm_passages({1: "A", 2: "B"})), {}
    )
